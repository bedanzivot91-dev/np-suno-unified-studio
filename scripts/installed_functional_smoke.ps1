...
Require-File $ffmpeg 'NP FFmpeg'
Require-File $ffprobe 'NP FFprobe'
...
$duration = 0.0
if (-not [double]::TryParse($durationRaw, [Globalization.NumberStyles]::Float, [Globalization.CultureInfo]::InvariantCulture, [ref]$duration)) {
    throw "Installed NP FFprobe returned invalid duration: $durationRaw"
}
if ($duration -lt 1.5 -or $duration -gt 2.5) { throw "Installed NP FFprobe returned unexpected duration: $duration" }
...
if ($LASTEXITCODE -ne 0 -or $fpOutput -notmatch 'FINGERPRINT=') {
    throw "Installed NP fpcalc did not produce a fingerprint. Output: $fpOutput"
}
Write-Host '[FUNCTIONAL 3/8] fpcalc produced a real audio fingerprint.' -ForegroundColor Green
...
try {
    ...
    try {
        $graphics.DrawString('TEST 123', $font, [System.Drawing.Brushes]::Black, 30, 55)
    } finally {
        $font.Dispose()
    }
...
Require-File $ocrImage 'Generated OCR image'
$ocrText = (& $tesseract $ocrImage stdout --psm 7 -l eng 2>&1 | Out-String).Trim()
if ($LASTEXITCODE -ne 0) { throw "Installed Tesseract OCR failed: $ocrText" }
if ($ocrText -notmatch '(?i)TEST' -or $ocrText -notmatch '123') { throw "Installed Tesseract did not read the generated text. OCR output: $ocrText" }
...
Write-Host '[FUNCTIONAL 6/8] Installed Suno AI environments -> real Python imports' -ForegroundColor Cyan
...
& $python -c "import faster_whisper, ctranslate2; print('installed transcription AI OK')"
if ($LASTEXITCODE -ne 0) { throw 'Installed Suno transcription AI import failed.' }
$env:PYTHONPATH = $stemsEnv
& $python -c "import audio_separator; from audio_separator.separator import Separator; print('installed stems AI OK')"
if ($LASTEXITCODE -ne 0) { throw 'Installed Suno stems AI import failed.' }
Remove-Item Env:PYTHONPATH -ErrorAction SilentlyContinue
Write-Host '[FUNCTIONAL 6/8] Both installed Suno AI environments import successfully.' -ForegroundColor Green
...
foreach ($component in @('stems','transcription','chromaprint')) {
    ...
    if ($null -eq $state -or -not $state.installed) {
        throw "Installed Suno reports advanced component unavailable: $component"
    }
}
$v3 = Invoke-RestMethod -Uri "$base/api/v3/status" -TimeoutSec 20
if (-not $v3.ok -or -not $v3.preflight.ok) {
    throw "Installed Suno v3 preflight is blocked: $($v3.preflight | ConvertTo-Json -Depth 8 -Compress)"
}
...
if ($web.StatusCode -ne 200 -or [string]::IsNullOrWhiteSpace($web.Content) -or $web.Content.Length -lt 1000) {
    throw 'Installed Suno web application was not served correctly.'
}
Write-Host "[FUNCTIONAL 8/8] Suno UI served successfully ($($web.Content.Length) chars)." -ForegroundColor Green

Write-Host 'INSTALLED FUNCTIONAL SMOKE PASSED: media, fingerprint, OCR, yt-dlp, Suno AI, Suno advanced API and Suno UI.' -ForegroundColor Green
